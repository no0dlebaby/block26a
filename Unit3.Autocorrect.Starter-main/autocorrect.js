 ...
 */
const isRelevant = (arr, string)=>{
  let bool

  for (let ii=0; ii<arr.length; ii++){
    if(string.includes(arr[ii])) {
    bool = true
...
const getDistanceByLength = (str1, str2)=> {
  const lengthOne = str1.length
  const lengthTwo = str2.length
  const diff = lengthOne - lengthTwo
 ...
 */
const getClosestWord = (ogWord, words, distanceFn, threshold) => {
  let closestWord = ogWord
  let minDistance = Infinity

  for (i = 0; i <words.length; i++){
    const distanceBetweenWords = distanceFn(ogWord, words[i])
    if(distanceBetweenWords < minDistance && distanceBetweenWords <= threshold){
      closestWord = words[i]
      minDistance = distanceBetweenWords
    }
  }
